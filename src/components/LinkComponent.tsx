        ...
        <>
            {listLinks?.map((value, index) => (
                <Link key={index} href={value.url} className={isAdminRoute
                    ...
                    : 'w-full border-2 shadow-black shadow-lg border-gray-800 bg-white rounded-2xl my-2 p-4 py-6 flex justify-center hover:scale-[1.02] hover:shadow-lg duration-200 ease-in-out'
                }>
                    <span className={isAdminRoute ? 'text-xs text-gray-500 font-bold' : 'text-sm text-gray-500 font-bold'}>{value.name}</span>
                </Link>
            ))}
        </>
    )
